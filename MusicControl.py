                    ...
                    while 1:
                        try:
                            ...
                            if a[0] == a[1] == 170 and a[2] == 32:
                                ...
                                # #     print(str(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))+"播放暂停")
                                # #     #requests.get(self.ip+"/gpio", params={"oper": "stopall"})
                                # #     # requests.get(self.ip + "/gpio", params={"oper": "startr"})
                                # #     self.is_close = True
                                # #     self.is_open = False
                                if self.is_open and c[32] < 35 and not self.is_close:
                                    print(str(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))) + "播放暂停")
                                    # requests.get(self.ip+"/gpio", params={"oper": "stopall"})
                                    # requests.get(self.ip + "/gpio", params={"oper": "startr"})
                                ...
                                if c[32] > 65 and not self.is_open and self.is_close:#and not flag:
                                    print(str(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))+"播放恢复")
                                ...
                                # highalpha = (a[16] << 16) | (a[17] << 8) | (a[18])
                                # print("low"+lowalpha)
                                # print("high"+highalpha)
                                #np.savetxt("data2.txt",self.vaul)
                                self.vaul = []

...
def main():
    ...
    CSV_FILE_PATH = 'D://pyproject//MusicControl//2020.5.02.csv'
    ...
    model.add(K.layers.Dense(units=5, input_dim=8, kernel_initializer=init, activation='relu'))
    ...
    x = np.array(train_x)
    y= np.array(train_y)
    h = model.fit(x, y, batch_size=b_size, epochs=max_epochs, shuffle=True, verbose=1)
    print("Training finished \n")

    # 4. 评估模型
    tx = np.array(test_x)
    ty = np.array(test_y)
    eval = model.evaluate(tx, ty, verbose=0)
